...
void ll_reversal(struct Node_prototype **head_node){

	struct Node_prototype* temp = *head_node;
	struct Node_prototype* prev = NULL;
	struct Node_prototype* current = *head_node;

	while(current != NULL){

		temp = current;
		current = temp->next_ptr;
		temp->next_ptr = prev;
		prev = temp;	
	}
...
int main(){
	...
	printf("Updated print_linked_list: \n");

	ll_reversal(&node_one);
